...
| G4 | `scripts/checks/index-drift.mjs` | `INDEX.md` matches what `gen-index` produces from `library.json` + component frontmatter (U8 compares the native manifests' `name` and `version` against `library.json`; `manifest.generated.json` is drift-checked by the house guard `check-self-consistency`, not by any spine check) - a hand-edited generated INDEX is an error | sec 2.6 (G4), sec 10.3 | no | Regenerate: `npx agent-skills-toolkit gen-index . --write`. Edit the source (frontmatter / library.json / gen-index), not INDEX.md. |
...
G3 is the **baseline**, and the check delivers the presence half of it: an eval set per chain edge and per hook event, plus the stale-edge regression signal. The clause's *execution* half (Standard sec 2.6 G3, "CI executes them") is checked by nothing in the spine. The Standard explicitly defers the multi-tier eval **engine** (Static / LLM-Judge / Monte-Carlo) to roadmap; that judging layer ships beside the gate as opt-in evidence, never inside the deterministic CI gate (Design Principle 3, ADR 0023).
