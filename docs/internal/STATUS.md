...
| Skills | **26** |
| Tests | **1576, 0 failures** (1572 pass, 4 skipped; local suite run **2026-09-17**). Up from 1506: thirteen for `U18` (`command-size-cap`), five for the `U6` root-relative fix, one pinning the collision severity ADR 0060 downgraded, and the rest from the eight audit patches - a symlink containment suite, two ReDoS timing assertions, a piped-stdout drain harness and a bin-help table reader. **Three of the four skips are the symlink cases**, which need a privilege Windows does not grant by default and run on the Linux legs in CI; the fourth is a POSIX-only path case. |
