...
- `publish-npm.yml` - the npm publish. A pushed `v*` tag reaches it, and so does an explicit `workflow_dispatch` (dry-run by default on that path). **A tag does not reach the registry on its own:** the `publish` job is bound to the `npm-publish` environment, whose required reviewer is the control that keeps publishing a deliberate act. Its last step asks the registry, from outside this workflow, whether the version the tag names actually arrived (`scripts/verify-published-version.mjs`). *(This entry said "workflow_dispatch-only ... never fires on a tag push" from v1.17.0 until the release-identity pass; the tag trigger was added in v1.17.0 and this description was never updated.)*
