...
| G4 | `scripts/checks/index-drift.mjs` | `INDEX.md` matches what `gen-index` produces from `library.json` + component frontmatter (U8 compares the native manifests' `name` and `version` against `library.json`; `manifest.generated.json` is drift-checked by nothing) - a hand-edited generated INDEX is an error | sec 2.6 (G4), sec 10.3 | no | Regenerate: `npx agent-skills-toolkit gen-index . --write`. Edit the source (frontmatter / library.json / gen-index), not INDEX.md. |
