...
- `standard-coverage.md` - every RFC-2119 `MUST` in `STANDARD.md` with the check that enforces it, the ADR ruling that dispositions it, or the stated gap. Generated by `scripts/gen-standard-coverage.mjs` and drift-guarded in `npm test`, so a `MUST` written without a disposition fails CI instead of joining the gap quietly (ADR 0059).
