...
export const DISPOSITIONS = Object.freeze({
  ...
  "sec 2.6 #1 a-plugin-claiming-gold-must-satisfy-every-item": { disposition: "checked", reqIds: ["G1", "G2", "G3", "G4", "G5", "G6", "G7", "G8", "G9", "G10"], note: "The ten Gold checks ARE the items below." },
  ...
  "sec 7.3 #1 each-component-must-carry-its-current-version-semver": { disposition: "ruled", adr0059: "sec 7.3 `HISTORY.md` at Silver+", reqIds: ["S3"], note: "Two halves. The `metadata.version` half is carried: S3 requires each components index entry to declare a version. The HISTORY.md half is ADR 0059's first ruling: 'sec 7.3 HISTORY.md at Silver+ - Demote to SHOULD - The requirement was aspirational when written and no adopter has met it, this repository included.' NOT YET APPLIED: the clause still reads MUST, and this repository declares Advanced with zero HISTORY.md files (verified 2026-09-18)." },
  ...
  "sec 7.5 #1 deprecated-components-must-still-validate-and-function-until": { disposition: "gap", note: "Every check grades a deprecated component exactly as an active one, so the clause holds by construction. Nothing tests it AS a requirement, and nothing would notice a check that started exempting them." },
  ...
  "sec 8.1 #2 anti-patterns-e-g-helps-with-x-must": { disposition: "partial", reqIds: ["U5"], note: "U5 scores the description and a bare 'Helps with X' falls below the bar, but no rule names the anti-pattern, and U5 emits a warn rather than an error." },
  ...
  "sec 12 #1 a-plugin-must-not-embed-a-marketplace-that": { disposition: "gap", note: "U17 checks the SHAPE of a marketplace.json. Nothing checks whether its entries list the plugin that ships it. ADR 0051 rules that a catalogue-level finding is never a numbered requirement on an individual plugin, which is why this is stated rather than graded, but no ADR has ruled this clause itself." },
