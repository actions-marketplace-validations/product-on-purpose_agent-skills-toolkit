...
| 7 | Standard-to-check coverage table, generated | **ADOPT - DONE** | [ADR 0059 (every MUST maps to a check, a SHOULD or a stated gap)](decisions/0059-every-must-maps-to-a-check-a-should-or-a-stated-gap.md), ruled 2026-09-08. The generated table is a test |
...
| ~~2026-09-18 to 09-20~~ | re-verify the two vendor probes | **SUPERSEDED.** Both probes were re-run and re-verified on **2026-09-17** (PR #327). The window closed early and correctly |
| ~~2026-09-23/24~~ | the probes go stale and block every release | **SUPERSEDED by the re-run.** Both `kind: "probe"` claims now read `verifiedOn: 2026-09-17`. With `FRESHNESS_DAYS = 30` and the staleness test `age > FRESHNESS_DAYS`, the window closes on **2026-10-17**, and a run on **2026-10-18** reports STALE and blocks every release from that morning |
