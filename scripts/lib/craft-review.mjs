...
export function phaseTwoEligible(gate) {
  ...
  if (g.exitCode === 2) {
    return { eligible: false, reason: "the deterministic gate exited 2, its operator-error code: the RUN was misconfigured (an askit.config.json that does not load, an unknown flag, an invalid --profile or --mode, or a root that is not a directory). There may be no gate findings to resolve; fix the run and re-run the gate" };
  }
  if (g.exitCode !== 0) {
