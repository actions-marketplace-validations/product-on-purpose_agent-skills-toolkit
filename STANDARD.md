...
> **Standard version 0.17**, amended 2026-09-20 (promoted to the repository root at v0.8 on 2026-05-26). This is the normative Standard that every tool in `agent-skills-toolkit` enforces; the version-history notes below record how the frozen draft converged.
> v0.17: **three warning windows close, and two clauses are relaxed to what the tooling can honestly promise. No new check, no tightening, and the spine stays at 35.** The three graduations are `G1` (a hook handler type the agent parses and silently skips), `G2` (CI that credits an EXECUTED gate rather than a mention) and `G8` (a folder README that exists but cannot be read). All three shipped warn-first with `until: "0.17"`, so they become gate-failing errors for a plugin that adopts this revision and change nothing for one that stays below it. **No cap metadata is deleted** - an expired cap is inert, and removing one would red-line every plugin still pinned beneath it. **`U18` is NOT a fourth graduation and never was**: it emits `warn` natively, so its cap capped a warn at warn, which is a no-op. That was measured at both pins with `G2` as a positive control and ruled in [ADR 0061](docs/internal/decisions/0061-u18-is-permanently-warn-and-the-strengthening-set-is-standard-0-18.md); the v0.16 note above is corrected in place rather than rewritten. **`G8`'s graduation carries ADR 0056's condition, and it was met by re-running the census rather than by trusting the date:** 514 `README.md` files in this repository and every folder README across all six reference-family members, **zero unreadable**. **Two relaxations ride this revision because a relaxation has no burndown** (sec 7.7): it can never make a passing plugin fail, so it adds nothing to the window ledger that made "graduations only" worth protecting. First, `G2`'s clause stops promising "CI that passes" - the check reads workflow text offline and cannot observe a run, a trigger firing or a green result, and making it ask a CI provider was refused because two people grading the same commit would get different answers (HC-09, the G2 clause-versus-check gap, ruled here). Second, the co-located `HISTORY.md` requirement drops from MUST to SHOULD: it was a MUST that no check read, and this Standard's own reference implementation violated it **35 times while its gate reported Gold** ([E67](docs/internal/backlog/enhancements.md)). Two sentences claiming "tooling enforces this" are deleted in the same change, because they were false at every revision that carried them.
> v0.16: **one check added and two existing checks tightened, all three warn-first and all three capped until 0.17.** The spine moves from 34 to **35**. **`U18` (`command-size-cap`, sec 3.2, ADR 0058)** is the new one: on a plugin that declares `codex` as an agent-target, a `commands/*.md` over the byte cap Codex applies when it migrates commands into skills is a `warn`. The vendor **SKIPS** an oversized command rather than truncating it - `codex-rs/core-plugins/src/command_migration.rs` does `continue`, so no skill is written and no error is raised - which makes this the only thing the spine grades whose symptom is a component's ABSENCE. The check measures the source file as a **declared proxy** for the rendered skill the vendor actually caps, and that is why it is a `warn` rather than an `error`: an error would assert a measurement it did not take. **`G1` (hook documentation)** now reports a hook handler type Codex parses and silently skips, and **`G2` (self-hosting CI)** now credits a gate that is EXECUTED rather than merely named; both shipped in the same train. **CORRECTED at 0.17 (ADR 0061): only `G1` and `G2` of the three actually graduate.** `U18` emits `warn` natively, so its `until: "0.17"` cap capped a warn at warn - a no-op that changed no severity at any pin, measured against a fixture with `G2` as a positive control. Its cap is removed and it is permanently `warn`, which is what ADR 0058 intended all along. The rest of this note stands. **All three carried `until: "0.17"` and never `until: "0.16"`**, because `scripts/lib/standard-ceiling.mjs` gates on `isAfter(migration.until, pinned)`: a cap expiring at the revision that introduces it is inert for every adopter of that revision, which would give a plugin adopting 0.16 zero migration window. `U18` additionally carries `since: "0.16"`, because `until` alone would leave a plugin pinned below 0.16 exposed to a check that did not exist at its pin. **Blast radius was measured for all three, not argued:** the six reference-family members were graded at the catalogue's pinned shas before and after each change, with zero movement. The `G2` tightening is additionally recorded as having had a WRONG first version - a rule that blanked quoted runs moved a member from passing to failing, and that member's CI was correct - which is why the measurement is the rule and the argument is not.
...
| G2 | **Self-hosting CI that invokes the gate.** The plugin ships a CI workflow that runs the full tier-applicable check suite via the portable scripts (Section 4). **Weakened from "and passes it" at v0.17 (HC-09 ruled):** the check reads workflow text offline and cannot observe a run, a trigger firing, or whether CI is green, so the clause now promises only what a deterministic offline grader can see. Making the gate ask a CI provider whether a run went green was refused - it would make two people grading the same commit get different answers. | self-hosting check (workflow text; NOT a live CI result) | - |
...
- **A command a target's migration would DROP is a defect of the plugin, not of the target.** On a plugin that declares `codex` as an agent-target, every `commands/*.md` SHOULD stay under the byte cap at which Codex's command migration refuses it (`U18`, `command-size-cap`). Codex migrates a plugin's commands into skills and **skips** any whose rendered skill exceeds `MAX_MIGRATED_COMMAND_SKILL_BYTES` (4000 bytes, read 2026-09-05 from `openai/codex` at `52e73e3a`): the loop moves on, no `SKILL.md` is written, and no error is raised anywhere. The command therefore does not exist on Codex, and absence has no symptom - which is why this is the requirement rather than a note. The cap applies to the RENDERED skill, so the check measures the source file as a declared proxy and reports at `warn`; asserting an exact rendered size would be claiming a measurement it did not take. Introduced at Standard 0.16 and **permanently `warn`** (ADR 0058, corrected by ADR 0061). It does **not** graduate, and no revision is scheduled to make it gate: the check measures the source file as a declared proxy for a rendered skill it never sees, and an error would assert a measurement it did not take. **This clause previously read "it gates from 0.17", which was false** - the check emits `warn` natively, so its `until: "0.17"` cap was a no-op that would have changed nothing when the window closed.
...
Within the agentskills.io arbitrary `metadata` map, this Standard defines these keys. `version` is REQUIRED on every component at every tier. A `HISTORY.md` is RECOMMENDED at Bronze and REQUIRED at Silver+ (Section 2.5); **when a `HISTORY.md` is present, `version` SHOULD equal its latest entry.** **No tooling checks this**, and an earlier revision of this line said it did (v0.17). The rest are RECOMMENDED for governance:
...
Each component MUST carry its current `version` (semver) in frontmatter (`metadata.version`) at every tier, and SHOULD maintain dated change notes in a **co-located history file** (`HISTORY.md` beside the component) **at Silver and above** (RECOMMENDED at Bronze). **Demoted to SHOULD at v0.17** (E67): it had been a requirement nothing checked, and one this Standard's own reference implementation broke 35 times while grading Gold. The frontmatter version is the machine-readable pointer; the history file is the human-readable record. When both exist, the frontmatter version SHOULD equal the latest `HISTORY.md` entry (Section 3.7). **Demoted alongside the clause above at v0.17**, and it is recorded that the first pass of that demotion missed this sentence - the coverage generator refused to build with a stale mapping, which is what caught it. The plugin-level CHANGELOG aggregates component histories; the frontmatter version, history file, and CHANGELOG SHOULD NOT contradict each other. **No tooling checks this**, and an earlier revision of this line said it did (v0.17).
